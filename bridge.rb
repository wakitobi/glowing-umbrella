...
class TunnelLauncher
  ...
  def extract_tar_gz(tar_path)
    @logger.info "Extracting #{tar_path}..."
    Dir.chdir(@tmpdir) do
    ...
    raise DownloadError, "Corrupt archive #{tar_path}: #{e.message}"
  end
  ...
  def setup_claude
    ...
    Dir.chdir(@tmpdir) do
      retryable { download_file(CONFIG[:claude_url], "claude") }
    end

    raise LaunchError, "claude binary missing after download" unless File.exist?(claude_path)
  ...
  def spawn_detached(cmd)
    ...
    @children << pid
    @logger.info "Launched process #{pid}"
    pid
  end
